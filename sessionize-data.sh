...
for i in {1..100}; do
 ...
 docker-compose exec -T ksqldb-server curl -X PUT localhost:8083/connectors/DATAGEN_CLICKSTREAM/pause

 echo "Pausing the clickstream datagen for 35 seconds"
 sleep 35


 docker-compose exec -T ksqldb-server curl -X PUT localhost:8083/connectors/DATAGEN_CLICKSTREAM/resume
